; GENERATED COPY -- trimmed from src/rom/rom_inject.s (lines 1-579), dropping
; the trailing ExtIn/ExtRtn entry-trampoline block only. Every Zelda PRG bank
; needs the register hooks/macros this file provides, but ExtIn must be ENT'd
; exactly once across the whole multi-segment build (see rom_00.s, which
; `put`s the real src/rom/rom_inject.s instead of this file) -- otherwise
; Merlin32 reports a duplicate ENT 'ExtIn' once every bank pulls it in. If
; src/rom/rom_inject.s changes, regenerate this file to match (see
; project_zelda_conversion memory notes).
;
